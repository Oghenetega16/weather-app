import logoUrl from "../../public/assets/images/logo.svg";
...
export default function App() {
    ...
    const handleUnitSelection = (property: string, unitIndex: number) => {
    ...
    return (
        <header className="flex justify-between">
            ...
            <div className="flex flex-col relative">
                ...
                {isDropdownOpen && (
                    <div className="bg-[#25253F] border border-neutral-600 px-2 py-3 rounded-lg shadow-xl absolute top-11 -left-21 w-45 z-10">
                        <p className="text-sm">Switch to Imperial</p>
                        <div>
                            {quantities.map((quantity, index) => (
                                <div key={index} className={`pt-1 pb-2 ${index < quantities.length - 1 ? 'border-b border-neutral-600' : ''}`}>
                                    <span className="capitalize text-xs text-neutral-300">{quantity.property}</span>
                                    <div className="space-y-2 text-sm">
                                        <div 
                                            className={`flex justify-between ${selectedUnits[quantity.key as keyof typeof selectedUnits] === 0 ? 'bg-[#2F2F49]' : 'bg-transparent'} p-1 rounded cursor-pointer`} 
                                            onClick={() => handleUnitSelection(quantity.key, 0)}
                                        >
                                            <p className="">{quantity.units[0].unit}</p>
                                            {selectedUnits[quantity.key as keyof typeof selectedUnits] === 0 && <img src={checkMark} alt="check icon" />}
                                        </div>
                                        <div 
                                            className={`flex justify-between ${selectedUnits[quantity.key as keyof typeof selectedUnits] === 1 ? 'bg-[#2F2F49]' : 'bg-transparent'} p-1 rounded cursor-pointer`} 
                                            onClick={() => handleUnitSelection(quantity.key, 1)}
                                        >
                                            <p className="">{quantity.units[1].unit}</p>
                                            {selectedUnits[quantity.key as keyof typeof selectedUnits] === 1 && <img src={checkMark} alt="check icon" />}
                                        </div>
                                    </div>
                                </div>
                            ))}
